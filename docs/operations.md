...
`drift-model-explain-grid` is centered on ml utilities work in Haskell. The technical target is: Create a Haskell reference implementation for explain workflows, centered on visual model generation, layout fixtures, and stable geometry snapshots.
...
The score is deterministic. The current threshold is 153. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 3.
